...
class AckRetryManager:
    def __init__(self, dispatcher, send_function, max_retries=3, retry_interval=5):
        self.dispatcher = dispatcher              # Handles event construction
        self.send_function = send_function        # Typically send_over_lora
    ...
    def _retry_loop(self):
        while self.running:
            ...
            for uid, message in retry_list:
                ...
                print(f"[AckRetry] Retry {message.retries} for UID {uid}")
                self.send_function(message.encoded_bytes)

            time.sleep(1)

    def _handle_failure(self, uid, message):
        print(f"[AckRetry] Max retries reached. Marking UID {uid} as FAILED.")
